    ...
    public class EnemyMovement4 : MonoBehaviour
    {
        ...
        void Update()
        {
            if (!playerMovement.isEnemy4Freeze)
            {
                ...
                Debug.Log("enemy size4" + enemysize);
                // Set the new sprite image and scale it to fit the current object size
                if (playersize.x < enemysize.x && playersize.y < enemysize.y && playerMovement.isEnemy4spiked == false)
                {
                    Vector3 targetPosition = player.transform.position;
                    Vector3 enemyPosition = transform.position;
                    float step = speed * Time.deltaTime;
                ...
                if (playersize.x < enemysize.x && playersize.y < enemysize.y && playerMovement.isEnemy4spiked == true)
                {
                    Vector3 targetPosition = player.transform.position;
                    Vector3 enemyPosition = transform.position;
                    float step = speed * Time.deltaTime;
