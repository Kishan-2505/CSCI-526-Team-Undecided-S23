...
namespace Level3_5
{
    public class EnemyMovement4 : MonoBehaviour
    {
        ...
        void Update()
        {
            if (!playerMovement.isEnemy4Freeze)
            {
                ...
                if (playersize.x < transform.localScale.x && playersize.y < transform.localScale.y && playerMovement.isEnemy4spiked == false) 
                {
                    Vector3 targetPosition = player.transform.position;
                    Vector3 enemyPosition = transform.position;
                    float step = speed * Time.deltaTime;
                ...
                else if(playerMovement.isEnemy4spiked==false)
                {
                ...
                if (playersize.x < transform.localScale.x && playersize.y < transform.localScale.y && playerMovement.isEnemy4spiked == true)
                {
                    Vector3 targetPosition = player.transform.position;
                    Vector3 enemyPosition = transform.position;
                    float step = speed * Time.deltaTime;
                ...
                else if (playerMovement.isEnemy4spiked == true)
                {
                    Vector3 direction = transform.position - player.transform.position;
                    transform.Translate(direction.normalized * speedReduced * Time.deltaTime);
        ...
        private void OnCollisionEnter2D(Collision2D collision)
        {
            if (collision.gameObject.tag == "Spike")
            {
                ...
                playerMovement.isEnemy4spiked = true;
