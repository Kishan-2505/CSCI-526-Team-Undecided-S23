    ...
    public class EnemyMovement5 : MonoBehaviour
    {
        ...
        void Update()
        {
            if (!playerMovement.isEnemy5Freeze)
            {
                ...
                Debug.Log("Enemy size5: " + enemysize);
                // Set the new sprite image and scale it to fit the current object size
                if (playersize.x < enemysize.x && playersize.y < enemysize.y && playerMovement.isEnemy5spiked==false)
                {
                    Vector3 targetPosition = player.transform.position;
                    Vector3 enemyPosition = transform.position;
                    float step = speed * Time.deltaTime;
                ...
                if (playersize.x < enemysize.x && playersize.y < enemysize.y && playerMovement.isEnemy5spiked==true)
                {
                    Vector3 targetPosition = player.transform.position;
                    Vector3 enemyPosition = transform.position;
                    float step = speed * Time.deltaTime;
